...
// === FIXED editMember (minimal override): PUT /api/users/updateUser/:id + proper FormData ===
...
(function(){
  ...
  return fetch(url, { method: 'HEAD', signal: ctrl.signal })
    .then(resp => {
      if (!resp || !resp.ok) return null; // endpoint missing → do not start EventSource
      try {
        ...
        es.onerror = function(){
          ...
          if (!window.__activityPollingStarted && typeof startPolling === 'function') startPolling();
        };
        return es;
      } catch(_){ return null; }
    })
