...
async function editMember(event) {
  ...
  const errData = await tryJson(response).catch(()=>({}));
  console.error('❌ Backend update error:', errData);
  showMessage(errData?.message || `Failed to update member (HTTP ${response.status})`, 'error');
  return;
}
            } catch (error) {
